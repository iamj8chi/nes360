...
AFRAME.registerComponent('wind-effect', {
  ...
  tick: function() {
    ...
    this.targets.forEach((target, i) => {
      ...
      // Position wiggle
      const px = this.easedSine(elapsed, this.data.strengthX, this.data.frequency, phase);
      const py = this.easedSine(elapsed, this.data.strengthY, this.data.frequency * 0.8, phase + 1);
      const pz = this.easedSine(elapsed, this.data.strengthZ, this.data.frequency * 1.2, phase - 1);
...
AFRAME.registerComponent('composite-tree', {
  ...
  createTree: function() {
    ...
    canopy.setAttribute('wind-effect', `strengthX: ${this.data.windStrength}; strengthY: ${this.data.windStrength * 0.5}; strengthZ: ${this.data.windStrength}`);
